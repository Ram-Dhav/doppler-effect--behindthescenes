...
# Smoothing: each new reading is blended with the previous smoothed value
# rather than used raw, so a single noisy/flickery frame doesn't cause a
# sudden jump in blink rate or pitch. Lower = smoother but slower to
# react; higher = more responsive but more jittery.
...
def run_demo():
    ...
    smoothed_area = 0.0  # exponential moving average of the hand area
    ...
    try:
        while True:
            ...
            # Blend the new reading into the running smoothed value instead
            # of reacting to it directly - reduces jitter from single
            # noisy frames while still tracking real movement quickly.
            ...
            print("raw={:.0f}  smoothed={:.0f}  detected={}  step={}/{}".format(
                raw_area, smoothed_area, detected, step, NUM_TONE_STEPS - 1))
