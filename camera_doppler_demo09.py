...
# ---------------------------------------------------------------------
PRINT_EVERY_N_FRAMES = 3   # print to console every Nth frame, not every frame
                            # (reduces SSH output lag; all frames still logged for the graph)
...
def play_tone_file(filename):
    """Play a WAV file WITHOUT blocking the main loop, so audio doesn't
    stall the LED/camera timing while it plays."""
    os.system("aplay -q {} 2>/dev/null &".format(filename))
...
def run_demo():
    ...
    try:
        while True:
            ...
            # Throttled console output - reduces SSH lag, all data still logged below
            if frame_count % PRINT_EVERY_N_FRAMES == 0:
                ...
                print("{}  detected={}  step={:2d}/{}  |  demo tone={:.0f} Hz  real shift={:+.5f} Hz".format(
                    bar, detected, step, NUM_TONE_STEPS - 1, demo_freq, real_shift_hz))
